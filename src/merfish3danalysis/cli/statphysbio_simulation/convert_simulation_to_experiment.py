...
@app.command()
def convert_simulation(root_path: Path) -> None:
    ...
    reshaped_simulation_data = simulation_data.reshape(
        num_rounds, num_ch, num_z, simulation_data.shape[-2], simulation_data.shape[-1]
    )
    ...
    for r_idx in range(num_rounds):
        tile_path = simulated_acq_path / Path(
            "data_r"
            + str(r_idx + 1).zfill(4)
            + "_tile"
            + str(fake_tile_id).zfill(4)
            + "_1"
        )
        tile_path.mkdir(exist_ok=True)
        image_path = tile_path / Path(
            "data_r"
            + str(r_idx + 1).zfill(4)
            + "_tile"
            + str(fake_tile_id).zfill(4)
        ...
        stage_metadata_path = simulated_acq_path / Path(
            "data_r"
            + str(r_idx + 1).zfill(4)
            + "_tile"
            + str(fake_tile_id).zfill(4)
