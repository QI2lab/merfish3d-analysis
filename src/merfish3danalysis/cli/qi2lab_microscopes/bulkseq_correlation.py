...
bulkseq_path = Path(
    "/mnt/opm3/merfish_panels/20240615/humanOB_bulk_RNASeq/SRR3376220/cufflinks/genes.fpkm_tracking"
)
...
segmented_results = (
    ...
    / Path("processed_v2")
    / Path("decoded")
    / Path("baysor_formatted_genes.csv")
)

merfish_df = pd.read_csv(segmented_results)
...
merged_df = pd.merge(
    merfish_counts_df, rnaseq_counts_df, on="gene_id", how="left", indicator=True
)
