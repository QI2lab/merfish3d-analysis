...
def _apply_first_polyDT_on_gpu(dr, gpu_id: int = 0):
    import cupy as cp
    import imagej
    import io
    import contextlib
    ...
    dr._datastore.save_local_registered_image(
        ref_image_decon, tile=dr._tile_id, deconvolution=True, round=dr._round_ids[0]
    )
...
def _apply_polyDT_on_gpu(dr, round_list: list, gpu_id: int = 0):
    """
        ...
        for r_idx, round_id in enumerate(round_list):
            ...
            if not (has_reg_decon_data) or dr._overwrite_registered:
                ...
                if dr._perform_optical_flow:
                    ...
                    data_registered = data_registered.clip(0, 2**16 - 1).astype(
                        np.uint16
                    )
