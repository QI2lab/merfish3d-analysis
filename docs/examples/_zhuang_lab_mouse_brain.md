...
From the overlap area, it appears that the `xy` order interpretation (on the right) is correct. To check more thoroughly, we can set line 167 in `00a_test_image_orientation.py` to `n_tiles=59`. This will plot the entire first tissue slice.
...
Once `02_register_and_deconvolve.py` is finished, you will need to create the correct cellpose settings. We have found that initially performing cellpose segmentation on a downsampled and maximum Z projected fiducial image is sufficient to seed Baysor for segmentation refinement.
